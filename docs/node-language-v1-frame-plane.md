# Node Language v1 ù?Frame & Plane
...
- Plane has a construction/reference origin and a unit normal direction
...
- Canonical validation: `PlaneData.canonical()` / `plane.normalized()` (see Reference Planes v1)
...
FRAME represents **position + orientation only** ù?not scale, not shear.
...
```text
FRAME      ù?Deconstruct Frame
FRAME_LIST ù?Deconstruct Frames
PLANE      ù?Deconstruct Plane
```
...
| FRAME | Origin, X, Y, Z | ù?|

```text
Plane ù?Frame requires X Hint (or deterministic fallback)
Frame ù?Plane via Deconstruct Frame (origin + Z normal)
```
...
| Box Face To Plane | `reference.planes.box_face_plane` |
...
- No Scale ù?scale belongs on geometry transform nodes
...
```text
Path Frames ù?FRAME_LIST ù?Transform Points by Frames
```
...
Removed producer ports are dropped from saved graphs. Old decomposed-frame connections to Transform Points by Frames cannot be auto-synthesized ù?repair manually using `FRAME_LIST`.
