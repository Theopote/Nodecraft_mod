...
- `Scatter On Surface Strip`（Sweep/Loft 曲面流）
  ...
  - 或 `scatter_surface_strip.output_blocks` -> `assign_block_type.input_coordinates` -> `apply_changes`
...
- `Offset Plane`（平行截面）
  ...
  - `offset_plane.output_plane` -> 接收 `PLANE` 的下游构建节点（如 profile/sweep/construct）

- `World Frame` + `Transform Frame`
  - `reference.frames.world_frame.output_origin/x_axis/y_axis/z_axis` -> `reference.frames.transform_frame` 对应输入
  - 可选：`reference.vectors.vector` -> `transform_frame.input_translation`
  - 可选：角度节点 -> `transform_frame.input_rotation_x/y/z`
  - `transform_frame.output_plane` -> `output.preview.preview_frame.input_plane`（或任何消费 frame/plane 的节点）
