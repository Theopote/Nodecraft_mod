...
public class SlopeMapNode extends BaseNode {
    ...
    public String getDescription() {
        return "Assigns flat/slope/steep materials using local height difference per X/Z column.";
    }
    ...
    public void processNode(@Nullable ExecutionContext context) {
        ...
        for (BlockPos pos : positions) {
            ...
            topYByColumn.merge(key, pos.getY(), Math::max);
        }
        ...
        for (BlockPos pos : positions) {
            int y = topYByColumn.getOrDefault(columnKey(pos.getX(), pos.getZ()), pos.getY());
            int dx = Math.abs(y - topYByColumn.getOrDefault(columnKey(pos.getX() + 1, pos.getZ()), y));
            int dz = Math.abs(y - topYByColumn.getOrDefault(columnKey(pos.getX(), pos.getZ() + 1), y));
            int grade = Math.max(dx, dz);
