...
/**
 * Ellipsoid (Blocks) 节点: 生成椭球体区域的坐标列表
 */
@NodeInfo(
    id = "spatial.generators.ellipsoid_blocks",
    displayName = "椭球体生成器",
    description = "生成椭球体区域的坐标列表，支持三轴独立半径",
...
public class EllipsoidBlocksNode extends BaseNode {
    ...
    public EllipsoidBlocksNode() {
        ...
        addInputPort(new BasePort(INPUT_CENTER_ID, "Center", "椭球体中心点", NodeDataType.BLOCK_POS, this));
        addInputPort(new BasePort(INPUT_RADIUS_X_ID, "Radius X", "X轴半径", NodeDataType.DOUBLE, this));
        addInputPort(new BasePort(INPUT_RADIUS_Y_ID, "Radius Y", "Y轴半径", NodeDataType.DOUBLE, this));
        addInputPort(new BasePort(INPUT_RADIUS_Z_ID, "Radius Z", "Z轴半径", NodeDataType.DOUBLE, this));
        ...
        addOutputPort(new BasePort(OUTPUT_BLOCKS_ID, "Blocks", "组成椭球体的方块列表", NodeDataType.BLOCK_LIST, this));
        addOutputPort(new BasePort(OUTPUT_COUNT_ID, "Count", "方块数量", NodeDataType.INTEGER, this));
    }

    @Override
    public String getDescription() {
        return "生成椭球体区域的坐标列表，支持三轴独立半径";
    }
            ...
            for (int dx = -maxR; dx <= maxR; dx++) {
                for (int dy = -maxR; dy <= maxR; dy++) {
                    for (int dz = -maxR; dz <= maxR; dz++) {
                        // 椭球方程: (x/rx)^2 + (y/ry)^2 + (z/rz)^2 <= 1
                        double dist = (dx * dx) / (rx * rx) + (dy * dy) / (ry * ry) + (dz * dz) / (rz * rz);

                        if (dist <= 1.0) {
                            if (isHollow) {
                                // 内椭球: 半径各减 shellThickness
