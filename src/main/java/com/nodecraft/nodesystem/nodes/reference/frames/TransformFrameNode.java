...
@NodeInfo(
    ...
    description = "Applies translation, Euler rotation (degrees), and positive uniform scale to a frame",
...
public class TransformFrameNode extends BaseNode {
    ...
    public TransformFrameNode() {
        ...
        addInputPort(new BasePort(INPUT_FRAME_ID, "Frame", "Optional packed frame (overrides decomposed origin/axes)", NodeDataType.FRAME, this));
        addInputPort(new BasePort(INPUT_ORIGIN_ID, "Origin", "Input frame origin point", NodeDataType.POINT, this));
        ...
        addOutputPort(new BasePort(OUTPUT_FRAME_ID, "Frame", "Transformed frame", NodeDataType.FRAME, this));
        addOutputPort(new BasePort(OUTPUT_ORIGIN_ID, "Origin", "Transformed frame origin", NodeDataType.POINT, this));
    ...
    public String getDescription() {
        return "Applies translation, Euler rotation (degrees), and positive uniform scale to a frame";
    }
    ...
    public void processNode(@Nullable ExecutionContext context) {
        ...
        if (frameObj instanceof FrameData frame) {
            ...
            zAxis = frame.getZAxis();
        } else {
            origin = FrameUtils.resolvePoint(inputValues.get(INPUT_ORIGIN_ID));
            if (origin == null) {
                origin = new Vector3d(0.0d, 0.0d, 0.0d);
            }
            xAxis = axisOrDefault(inputValues.get(INPUT_X_AXIS_ID), 1.0d, 0.0d, 0.0d);
            yAxis = axisOrDefault(inputValues.get(INPUT_Y_AXIS_ID), 0.0d, 1.0d, 0.0d);
            zAxis = axisOrDefault(inputValues.get(INPUT_Z_AXIS_ID), 0.0d, 0.0d, 1.0d);
        }

        if (!FrameUtils.isFinite(origin) || xAxis == null || yAxis == null || zAxis == null) {
            writeInvalid();
            return;
        }
        ...
        FrameData outFrame = new FrameData(outOrigin, outX, outY, outZ);

        outputValues.put(OUTPUT_FRAME_ID, outFrame);
        outputValues.put(OUTPUT_ORIGIN_ID, new PointData(outOrigin));
    ...
    private Vector3d axisOrDefault(Object axisObj, double x, double y, double z) {
        ...
        Vector3d resolved = FrameUtils.resolveVector(axisObj);
        if (resolved != null && FrameUtils.isUsableAxis(resolved)) {
            return resolved;
        }
        return null;
    }
