...
@NodeInfo(
    ...
    description = "Splits a list into two using a BOOLEAN_LIST mask (one boolean per item).",
    category = "math.list"
)
public class DispatchListNode extends BaseNode {
    ...
    public DispatchListNode() {
        ...
        IPort conditionInput = new BasePort(INPUT_CONDITION_ID, "Mask",
                "Boolean mask (one boolean per item)", NodeDataType.BOOLEAN_LIST, this);
        addInputPort(conditionInput);

        IPort trueOutput = new BasePort(OUTPUT_TRUE_LIST_ID, "True List",
                "Items for which the mask was true", NodeDataType.LIST, this);
        addOutputPort(trueOutput);

        IPort falseOutput = new BasePort(OUTPUT_FALSE_LIST_ID, "False List",
                "Items for which the mask was false", NodeDataType.LIST, this);
        ...
        if (listObj instanceof List<?> inputList && conditionObj instanceof List<?> conditionList) {
            for (int i = 0; i < inputList.size(); i++) {
                ...
                if (i < conditionList.size()) {
                    ...
                    if (condObj instanceof Boolean bool) {
                        condition = bool;
                    } else {
    ...
    public void setNodeState(Object state) {
        ...
        if (use instanceof Boolean value) {
            setUseDefaultValue(value);
        }
        Object defaultVal = stateMap.get("defaultValue");
        if (defaultVal instanceof Boolean value) {
