...
/** Rule hint chrome for {@link TagRelayNode}. */
public final class TagRelayActionProvider implements NodeActionProvider {
    ...
    public boolean render(INode node, Supplier<NodeGraph> graphSupplier) {
        if (!(node instanceof TagRelayNode)) {
            return false;
        }

        ImGui.text("Tag Relay Rules");
        ImGui.textWrapped(
                "Color supports #RRGGBB/#AARRGGBB, named tokens (danger/warn/io/math/flow/debug), or auto by tag keywords.");
